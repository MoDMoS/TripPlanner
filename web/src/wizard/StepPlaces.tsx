...
export function StepPlaces({ trip, onChanged, onContinue }: Props) {
  ...
  return (
    <div className="grid min-h-[70vh] gap-4 lg:grid-cols-[280px_minmax(0,1fr)_260px]">
      <aside className="space-y-4 rounded-xl border border-slate-700 bg-slate-900/70 p-4">
        <div>
          ...
          <ul className="mt-2 max-h-56 space-y-2 overflow-auto text-sm">
            {hits.map((hit) => (
              ...
              >
                ...
                >
                  <div className="font-medium text-slate-100">{hit.name}</div>
                  <div className="text-xs text-slate-400">{hit.address}</div>
                </button>
                <button
                  type="button"
        ...
        {preview ? (
          <div className="rounded-lg border border-slate-700 p-3 text-sm">
            ...
            <div className="text-xs text-slate-400">{preview.address}</div>
      ...
      <section className="min-h-[320px] overflow-hidden rounded-xl border border-slate-700">
        <TripMap places={mapPlaces} focus={focus} />
      </section>

      <aside className="flex flex-col rounded-xl border border-slate-700 bg-slate-900/70 p-4">
