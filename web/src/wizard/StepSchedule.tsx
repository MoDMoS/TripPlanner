...
export function StepSchedule({ trip, onChanged, onBack, onContinue }: Props) {
  ...
  if (!day) {
    return <p className="text-violet-300/80">สร้างวันและจัดสถานที่ก่อน</p>;
  }
  ...
  return (
    <div className="space-y-4">
        ...
        {days.map((d) => (
          <button
            ...
            className={`rounded-full px-3 py-1 text-sm ${
              d.id === day.id ? 'bg-violet-500 text-violet-950' : 'bg-violet-900/70 text-violet-200'
            }`}
      ...
      <section className="grid gap-4 rounded-xl border border-violet-500/25 bg-violet-950/40 p-4 lg:grid-cols-2">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          type="button"
          disabled={busy}
          className="rounded-lg border border-violet-500/35 px-3 py-2 text-sm"
      ...
      {timeline ? (
        <pre className="overflow-auto rounded-lg border border-violet-500/30 bg-violet-950/50 p-3 text-xs text-violet-200">
          {timeline}
        </pre>
      ) : null}
